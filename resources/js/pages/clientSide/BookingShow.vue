...
const goBack = () => {
  // Check if user is authenticated to determine which route to go back to
  if (user.value) {
...
const contactHost = () => {
  ...
  if (!user.value) {
    router.visit(`/login?redirect=/vehicles/${props.vehicle.id}`);
    return;
  }
...
const handleBookingAttempt = () => {
  if (!user.value) {
    // Save current vehicle page to redirect back after login
    router.visit(`/login?redirect=/vehicles/${props.vehicle.id}`);
  }
};
</script>
...
<template>
  ...
  <div class="min-h-screen bg-gradient-to-br from-neutral-50 to-neutral-100">
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
      ...
      <div v-if="!user" class="mb-6 bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-[#0081A7] rounded-lg p-4 shadow-md">
        <div class="flex items-start gap-3">
          <AlertCircle class="w-5 h-5 text-[#0081A7] flex-shrink-0 mt-0.5" />
          <div class="flex-1">
            <p class="text-sm text-neutral-800">
              <span class="font-semibold text-[#0081A7]">Sign in to book this vehicle.</span> 
              Create an account or log in to make a reservation and contact the host.
            </p>
            <div class="flex gap-2 mt-3">
              <Link 
                :href="`/login?redirect=/vehicles/${vehicle.id}`"
                class="px-4 py-2 bg-[#0081A7] text-white rounded-lg text-sm font-medium hover:bg-[#0081A7]/90 transition-colors shadow-sm"
              >
                Sign In
              </Link>
              <Link 
                :href="`/register?redirect=/vehicles/${vehicle.id}`"
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  ...
                  >
                    ...
                    {{ user ? 'Contact Host' : 'Sign In to Contact Host' }}
                  </button>
                </div>
              </div>
            </CardContent>
