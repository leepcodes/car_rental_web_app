...
<template>
  <Card 
    :class="[
      'overflow-hidden transition-all duration-300 bg-white border-2 border-[#0081A7] hover:border-[#00AFB9]',
  ...
  >
    ...
    >
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex-1 min-w-0">
            <CardTitle :class="[
              ...
              vehicle.available ? 'text-neutral-900 group-hover:text-[#0081A7]' : 'text-neutral-400'
            ]">
              {{ vehicle.name }}
            </CardTitle>
      ...
      <CardContent class="pb-3">
        ...
        <template v-else>
          <!-- Host Info -->
          <div class="flex items-center gap-2 mb-3 pb-3 border-b border-neutral-200">
            <div class="w-8 h-8 rounded-full bg-[#0081A7]/10 flex items-center justify-center">
              <Car class="w-4 h-4 text-[#0081A7]" />
            </div>
            <div class="flex-1 min-w-0">
          ...
          <div class="grid grid-cols-3 gap-2 text-center">
            <div class="flex flex-col items-center">
              ...
              <span class="text-xs text-neutral-700">{{ vehicle.passengers }}</span>
            </div>
            <div class="flex flex-col items-center">
              <Settings class="w-4 h-4 text-[#0081A7] mb-1" />
              <span class="text-xs text-neutral-700 truncate">{{ vehicle.transmission }}</span>
            </div>
            <div class="flex flex-col items-center">
              <Fuel class="w-4 h-4 text-[#0081A7] mb-1" />
              <span class="text-xs text-neutral-700">{{ vehicle.fuel }}</span>
            </div>
          </div>
        </template>
      </CardContent>
    ...
    <CardFooter class="pt-0 flex items-center justify-between">
      ...
      <Button 
        ...
        :class="[
          'transition-all',
          vehicle.available 
            ? 'bg-gradient-to-r from-[#0081A7] to-[#00AFB9] text-white font-bold hover:shadow-lg hover:scale-105' 
