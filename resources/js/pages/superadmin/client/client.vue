...
import ClientView from '@/pages/superadmin/client/ClientModal/clientview.vue'
...
const deleteFromDatabase = (userId: number): void => {
  router.delete(`/superadmin/clients/${userId}`, {
...
<template>
  <AppSidebarLayout>
    <template #default>
      ...
      <ClientView :isOpen="isModalOpen" :userId="selectedUserId" @close="closeModal" />
      ...
      >
        <div
          v-if="showUndo"
          class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-lg shadow-2xl flex items-center gap-4 z-50 min-w-[320px]"
        >
          ...
          <span class="flex-1">Client deleted</span>
